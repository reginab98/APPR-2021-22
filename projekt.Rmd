...
graf5
```
Med obdelavo podatkov sem opazila, da je za nekatere države opazna sezonskost. Primer je Grčija. Spodaj je na petem grafu prikazano število mesečnih odhodov v Grčijo za obdobje enajstih let. Vsako leto je očitno povečano število potnikov nekje sredi koledarskega leta. Na šestem grafu pa so od bližje prikazana tri leta in opazimo, da je povečano število potnikov v poletnih mesecih, kar je skladno s tem, da veliko potnikov z Brnika poleti tam dopustuje.
```{r graf6, fig.align='center', out.width = "70%"}
graf6
```

```{r graf7, fig.align='center', out.width = "70%"}
graf7
```
...
```
Za odhode potnikov v Grčijo sem imela podatke do sredine leta 2022, vendar je imela zadnje tri leta epidemija koronavirusa zelo velik vpliv na število potnikov. Iz podatkov do leta 2019 sem ocenila, koliko potnikov bi po mesecih v letu 2020 odletelo z Brnika, če ne bi bilo epidemije. Rezultati so spodaj prikazani na sedmem grafu.
```{r graf8, fig.align='center', out.width = "70%"}
graf8
```

## Zaključek

Tekom projektne naloge sem izvedela veliko novih informacij o letalskemu potniškemu prometu iz Slovenije. Ideja za nadaljno analizo bi bila na primer izbrati podatke o končnih destinacijah potnikov, ki odletijo z Brnika, saj je to manjše letališče in v veliko primerih potniki letijo do kakšnega večjega letališča in od tam do končne destinacije. Tako bi lahko zares analizirali, kam potujejo potniki z ljubljanskega letališča.
